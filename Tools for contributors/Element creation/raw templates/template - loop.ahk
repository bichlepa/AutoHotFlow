...
Element_run_&ElementType&_&Name&(Environment, ElementParameters)
{
	...
	MsgBox, 4, Choose, Continue or not
	IfMsgBox yes
		...
		; start loop if the window exists
		if windowID
		{
			x_SetVariable(Environment, "A_WindowID", windowID, "loop")
			x_SetVariable(Environment, "A_Index", 1, "loop")
			x_finish(Environment, "head")
		}
		Else
		...
		gui, %guiID%: add, button, g&ElementType&_&Name&_OnCloseContinue, Continue
		gui, %guiID%: add, button, g&ElementType&_&Name&_OnCloseBreak, Break
	...
	else if (entryPoint = "Tail") ;Continue loop
	{
		...
		winID := x_getVariable(Environment, "A_WindowID")
		; start loop if the window exists
		if winexist("ahk_id " winID)
		{
			x_SetVariable(Environment, "A_WindowID", winID, "loop")
			x_SetVariable(Environment, "A_Index", 1, "loop")
			x_finish(Environment, "head")
		}
		Else
	...
	else
	{
		...
		x_finish(Environment, "exception", lang("No information whether the connection leads to head or tail"))
	}
}
...
&ElementType&_&Name&_OnCloseBreak()
{
	Environment := x_GetMyEnvironmentFromExecutionID(A_Gui)
	gui, destroy
	x_finish(Environment, "tail")
...
{
	if (values.result = "head")
	{
		; write value to a thread variable
		x_SetVariable(Environment, "A_text", values.enteredText, "loop")
		x_finish(Environment, "head", values.message)
	}
	else
	{
		x_finish(Environment, "tail", lang("Unknown error"))
	}

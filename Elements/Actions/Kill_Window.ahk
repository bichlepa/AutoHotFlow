...
runActionKill_Window(InstanceID,ElementID,ElementIDInInstance)
{
	...
	local tempWinTitle
	local tempWinText
	local tempExcludeTitle
	local tempExcludeText
	local tempTitleMatchMode
	local tempahk_class
	local tempahk_exe
	local tempahk_id
	local tempahk_pid
...
getNameActionKill_Window()
{
	return lang("kill_Window")
}
getCategoryActionKill_Window()
{
	return lang("Window") "|" lang("Process")
}

getParametersActionKill_Window()
{
	global
...
GenerateNameActionKill_Window(ID)
{
	global
	
	local tempNameString:=lang("Close_Window")

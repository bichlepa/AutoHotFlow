...
RIni_Write(RVar, File, Newline="`r`n", Write_Blank_Sections=1, Write_Blank_Keys=1, Space_Sections=1, Space_Keys=0, Remove_Valuewlines=0, Overwrite_If_Exists=1, Addwline_At_End=0)
{
	...
	IfExist, %File%
	{
		...
		T_Time := A_Now
		If A_IsUnicode
			FileAppend, %Write_Ini%, %A_Temp%\%T_Time%.ini, UTF-16 
		Else
		...
		If A_IsUnicode
			FileAppend, %Write_Ini%, %File%, UTF-16 
		Else
...
RIni_GetKeyValue(RVar, Sec, Key, Default_Return="nothingNichtsНичего") ;Modified by bichlepa
{
	Global
	
	If (!RIni_%RVar%_Is_Set)
		Return Default_Return = "nothingNichtsНичего" ? -10 : Default_Return
	Sec := RIni_CalcMD5(Sec)
	
	If !RIni_%RVar%_%Sec%_Is_Set
		Return Default_Return = "nothingNichtsНичего" ? -2 : Default_Return

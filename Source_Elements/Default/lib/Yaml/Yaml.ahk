...
license info:
{
	"name": "Yaml Parser for AutoHotkey",
	"author": "HotKeyIt",
...
*/
Default_Lib_Yaml(YamlText,IsFile=1,YamlObj=0){ ; Version 1.0.0.17 http://www.autohotkey.com/forum/viewtopic.php?t=70559
  static
  static base:={Dump:"Default_Lib_Yaml_Dump",Save:"Default_Lib_Yaml_Save",Add:"Default_Lib_Yaml_Add",Merge:"Default_Lib_Yaml_Merge",__Delete:"__Delete",_Insert:"_Insert",_Remove:"_Remove",_GetCapacity:"_GetCapacity",_SetCapacity:"_SetCapacity",_GetAddress:"_GetAddress",_MaxIndex:"_MaxIndex",_MinIndex:"_MinIndex",_NewEnum:"_NewEnum",_HasKey:"_HasKey",_Clone:"_Clone",Insert:"Insert",Remove:"Remove",GetCapacity:"GetCapacity",SetCapacity:"SetCapacity",GetAddress:"GetAddress",MaxIndex:"MaxIndex",MinIndex:"MinIndex",NewEnum:"NewEnum",HasKey:"HasKey",Clone:"Clone",base:{__Call:"Default_Lib_Yaml_Call"}}
  ...
  Loop,Parse,YamlText,`n,`r
  {
    If (!_CNT && (A_LoopField=""||RegExMatch(A_LoopField,"^\s+$"))){ ;&&__KEY=""&&__SEQ="")){
			If ((OBJ:=LVL%__LVL%[""].MaxIndex())&&IsObject(LVL%__LVL%["",OBJ])&&__SEQ){
				If (__KEY!="")
					Default_Lib_Yaml_Continue(LastContObj:=LVL%__LVL%["",Obj],LastContKEY:=__key,"",__SCA)
				else Default_Lib_Yaml_Continue(LastContObj:=LVL%__LVL%[""],LastContKEY:=Obj,"",__SCA,__SEQ)
			} else If (__SEQ && OBJ){
				Default_Lib_Yaml_Continue(LastContObj:=LVL%__LVL%[""],LastContKEY:=OBJ,"",__SCA,__SEQ)
			} else If (OBJ){
				Default_Lib_Yaml_Continue(LastContObj:=LVL%__LVL%[""],LastContKEY:=OBJ,"",__SCA,1)
			} else if (__KEY!="")
				Default_Lib_Yaml_Continue(LastContObj:=LVL%__LVL%,LastContKEY:=__KEY,"",__SCA)
			else LinesAdded--
			LinesAdded++
      Continue
    ...
    If InStr(A_LoopField,"#"){
      If (RegexMatch(A_LoopField,"^\s*#.*") || InStr(A_LoopField,"%YAML")=1) ;Comments only, do not parse
        continue
      else if Default_Lib_Yaml_IsQuoted(LTrim(A_LoopField,"- ")) || RegExMatch(A_LoopField,"(---)?\s*?(-\s)?("".+""\s*:\s|'.+'\s*:\s|[^:""'\{\[]+\s*:\s)\s*([\|\>][+-]?)?\s*(!!\w+\s)?\s*("".+|'.+)$")&&!RegExMatch(A_LoopField,"[^\\]""\s+#")
    ...
    If _CNT {
      If Default_Lib_Yaml_IsSeqMap(RegExReplace(IncompleteSeqMap LoopField,"^(\s+)?(-\s)?("".+""\s*:\s|'.+'\s*:\s|[^:""'\{\[]+\s*:\s)?"))
    ...
    _KEY:=Default_Lib_Yaml_UnQuoteIfNeed(_KEY)
    If IsVal:=Default_Lib_Yaml_IsQuoted(_VAL)
			_VAL:=Default_Lib_Yaml_UnQuoteIfNeed(_VAL)
    ;determine current level
    _LVL:=Default_Lib_Yaml_S2I(_LVL)
    ...
    _LFL:=Default_Lib_Yaml_UnQuoteIfNeed(_LFL)
    ...
    If (!IsVal && !_CNT && (_CNT:=Default_Lib_Yaml_Incomplete(Trim(_LFL))||Default_Lib_Yaml_Incomplete(Trim(_VAL)))){
      IncompleteSeqMap:=LoopField
      continue
    }
    ...
    If (__CNT)||(_LVL>__LVL&&(__KEY!=""&&_KEY="")&&(__VAL!=""||__SCA))||(__SEQ&&__SCA)
      _KEY:="",_VAL:=""
    If (__CNT||(_LVL>__LVL&&(__KEY!=""||(__SEQ&&(__LFL||__SCA)&&!Default_Lib_Yaml_IsSeqMap(__LFL)))&&!(_SEQ||_KEY!=""))){
			If ((OBJ:=LVL%__LVL%[""].MaxIndex())&&IsObject(LVL%__LVL%["",OBJ])&&__SEQ){
        If __KEY!=
          Default_Lib_Yaml_Continue(LVL%__LVL%["",Obj],__key,_LFL,__SCA),__CNT:=Default_Lib_Yaml_SeqMap(LVL%__LVL%["",OBJ],__KEY,LVL%__LVL%["",OBJ,__KEY])?"":__CNT
        else Default_Lib_Yaml_Continue(LVL%__LVL%[""],Obj,_LFL,__SCA,__SEQ),__CNT:=Default_Lib_Yaml_SeqMap(LVL%__LVL%[""],OBJ,LVL%__LVL%["",OBJ],__SEQ)?"":__CNT
      } else If (__SEQ && OBJ){
        Default_Lib_Yaml_Continue(LVL%__LVL%[""],Obj,_LFL,__SCA,__SEQ)
        __CNT:=Default_Lib_Yaml_SeqMap(LVL%__LVL%[""],OBJ,LVL%__LVL%["",OBJ],__SEQ)?"":__CNT
      } else If (OBJ && __KEY=""){
        Default_Lib_Yaml_Continue(LVL%__LVL%[""],OBJ,_LFL,__SCA,1)
        __CNT:=Default_Lib_Yaml_SeqMap(LVL%__LVL%[""],OBJ,LVL%__LVL%["",OBJ],1)?"":__CNT
      } else {
        Default_Lib_Yaml_Continue(LVL%__LVL%,__KEY,_LFL,__SCA)
        __CNT:=Default_Lib_Yaml_SeqMap(LVL%__LVL%,__KEY,LVL%__LVL%[__KEY])?"":__CNT
      }
      Continue
    }
    ...
    If (__SEQ&&(_LVL>__LVL)&&_KEY!=""&&__KEY!=""){
      ...
      If _SEQ {
          If !Default_Lib_Yaml_SeqMap(LVL%_LVL%["",OBJ,__KEY,""],_KEY,_VAL){
      ...
      } else If !Default_Lib_Yaml_SeqMap(LVL%_LVL%["",OBJ],_KEY,_VAL){
      ...
      If (__SEQ && _LVL>__LVL) {
        If (OBJ:=LVL%_PRV%[""].MaxIndex())&&IsObject(LVL%_PRV%["",OBJ]){
          If !Default_Lib_Yaml_SeqMap(LVL%_PRV%["",OBJ],_KEY,_VAL)
          ...
          Default_Lib_Yaml_SeqMap(LVL%_PRV%["",OBJ?OBJ+1:1],_KEY,_VAL)
        }
      } else
        If !Default_Lib_Yaml_SeqMap(LVL%_LVL%,_KEY,_VAL)
          LVL%_LVL%[_KEY]:=_VAL!=""?_VAL:(LVL%_NXT%:=Object("base",base))
    } else if (_LVL>__LVL && (__KEY!="")) {
      If (__VAL!="" || __SCA){
        Default_Lib_Yaml_Continue(LVL%__LVL%,__KEY,_LFL,__SCA)
        Default_Lib_Yaml_SeqMap(LVL%__LVL%,__KEY,LVL%__LVL%[__KEY])
        Continue
      } else {
        If !Default_Lib_Yaml_SeqMap(LVL%__LVL%[__KEY],_KEY,_VAL) ;!!! no Scalar???
      ...
      If (_LVL>__LVL&&(OBJ:=LVL%__LVL%[""].MaxIndex())&&IsObject(LVL%__LVL%["",OBJ])&&__SEQ){
        If __CNT
          Default_Lib_Yaml_Continue(LVL%__LVL%[""],LVL%__LVL%[""].MaxIndex(),_LFL,__SCA,1)
        If (__CNT:=Default_Lib_Yaml_SeqMap(LVL%__LVL%[""],"",_LFL)?"":1)
        ...
        If __CNT
          Default_Lib_Yaml_Continue(LVL%__LVL%[""],LVL%__LVL%[""].MaxIndex(),_LFL,__SCA,1)
        If (__CNT:=Default_Lib_Yaml_SeqMap(LVL%_LVL%[""],"",_LFL)?"":1)
          LVL%_LVL%[""].Insert(_LFL)
      }
      Continue
    }
...
Default_Lib_Yaml_Save(obj,file,level=""){
...
Default_Lib_Yaml_Call(NotSupported,f,p*){
  If (p.MaxIndex()>1){
    Loop % p.MaxIndex()
      If A_Index>1
        f:=f[""][p[A_Index-1]]
...
Default_Lib_Yaml_Merge(obj,merge){
  for k,v in merge
  {
    If IsObject(v){
      If obj.HasKey(k){
        If IsObject(obj[k])
          Default_Lib_Yaml_Merge(obj[k],v)
        else obj[k]:=v
      } else obj[k]:=v
    } else obj[k]:=v
  }
}
Default_Lib_Yaml_Add(O,Yaml="",IsFile=0){
  static base:={Dump:"Default_Lib_Yaml_Dump",Save:"Default_Lib_Yaml_Save",Add:"Default_Lib_Yaml_Add",Merge:"Default_Lib_Yaml_Merge",__Delete:"__Delete",_Insert:"_Insert",_Remove:"_Remove",_GetCapacity:"_GetCapacity",_SetCapacity:"_SetCapacity",_GetAddress:"_GetAddress",_MaxIndex:"_MaxIndex",_MinIndex:"_MinIndex",_NewEnum:"_NewEnum",_HasKey:"_HasKey",_Clone:"_Clone",Insert:"Insert",Remove:"Remove",GetCapacity:"GetCapacity",SetCapacity:"SetCapacity",GetAddress:"GetAddress",MaxIndex:"MaxIndex",MinIndex:"MinIndex",NewEnum:"NewEnum",HasKey:"HasKey",Clone:"Clone",base:{__Call:"Default_Lib_Yaml_Call"}}
  If Default_Lib_Yaml_IsSeqMap(Trim(Yaml)){
    If !IsObject(O[""])
      O[""]:=Object("base",base)
    Default_Lib_Yaml_SeqMap(O[""],"",Yaml)
  } else Default_Lib_Yaml(Yaml,IsFile,O)
}
Default_Lib_Yaml_Dump(O,J="",R=0,Q=0){
  ...
  If IsObject(O[""]){
    ...
    If O[""].MaxIndex()
      for key, value in O[""]
      {
        ...
        If (J!=""&&J<=R){
          dump.=(F?(%F%1 Default_Lib_Yaml_Dump(value,J,R+1,F) %F%2):Default_Lib_Yaml_EscIfNeed(value)) (I=MX&&!M?E:C) ;(Q="S"&&I=1?S1:E)(Q="S"&&I=MX?S2:E)
        } else if F,dump:=dump N Default_Lib_Yaml_I2S(R) S
          dump.= (J!=""&&J<=(R+1)?%F%1:E) Default_Lib_Yaml_Dump(value,J,R+1,F) (J!=""&&J<=(R+1)?%F%2:E)
        else {
          ; If RegexMatch(value,"[\x{007F}-\x{FFFF}""\{\[']|:\s|\s#")
            dump .= Default_Lib_Yaml_EscIfNeed(value)
          ; else {
  ...
  for key, value in O
  {
    ...
    If (J=0&&!R)
      dump.= M1
    If (J!=""&&J<=R){
      dump.=(Q="S"&&I=1?M1:E) Default_Lib_Yaml_EscIfNeed(key) K
      dump.=F?(%F%1 Default_Lib_Yaml_Dump(value,J,R+1,F) %F%2):Default_Lib_Yaml_EscIfNeed(value)
      dump.=(Q="S"&&I=M?M2:E) (J!=0||R?(I=M?E:C):E)
    } else if F,dump:=dump N Default_Lib_Yaml_I2S(R) Default_Lib_Yaml_EscIfNeed(key) K
      dump.= (J!=""&&J<=(R+1)?%F%1:E) Default_Lib_Yaml_Dump(value,J,R+1,F) (J!=""&&J<=(R+1)?%F%2:E)
    else {
      ; If RegexMatch(value,"[\x{007F}-\x{FFFF}""\{\['\t]|:\s|\s#")
        dump .= Default_Lib_Yaml_EscIfNeed(value)
...
Default_Lib_Yaml_UniChar( string ) {
...
Default_Lib_Yaml_CharUni( string ) {
...
Default_Lib_Yaml_I2S(idx){
  Loop % idx
    str .= "  "
  Return str
}
Default_Lib_Yaml_Continue(Obj,key,value,scalar="",isval=0){
  ...
  obj[key]:=Default_Lib_Yaml_UnQuoteIfNeed(v temp)
}
Default_Lib_Yaml_Quote(ByRef L,F,Q,B,ByRef E){
  Return (F="\"&&!E&&(E:=1))||(E&&!(E:=0)&&(L:=L ("\" F)))
}
Default_Lib_Yaml_SeqMap(o,k,v,isVal=0){
  v:=Trim(v,A_Tab A_Space "`n"),m:=SubStr(v,1,1) SubStr(v,0)
  If Default_Lib_Yaml_IsSeqMap(v)
    return m="[]"?Default_Lib_Yaml_Seq(o,k,SubStr(v,2,StrLen(v)-2),isVal):m="{}"?Default_Lib_Yaml_Map(o,k,SubStr(v,2,StrLen(v)-2),isVal):0
}
Default_Lib_Yaml_Seq(obj,key,value,isVal=0){
  static base:={Dump:"Default_Lib_Yaml_Dump",Save:"Default_Lib_Yaml_Save",Add:"Default_Lib_Yaml_Add",Merge:"Default_Lib_Yaml_Merge",__Delete:"__Delete",_Insert:"_Insert",_Remove:"_Remove",_GetCapacity:"_GetCapacity",_SetCapacity:"_SetCapacity",_GetAddress:"_GetAddress",_MaxIndex:"_MaxIndex",_MinIndex:"_MinIndex",_NewEnum:"_NewEnum",_HasKey:"_HasKey",_Clone:"_Clone",Insert:"Insert",Remove:"Remove",GetCapacity:"GetCapacity",SetCapacity:"SetCapacity",GetAddress:"GetAddress",MaxIndex:"MaxIndex",MinIndex:"MinIndex",NewEnum:"NewEnum",HasKey:"HasKey",Clone:"Clone",base:{__Call:"Default_Lib_Yaml_Call"}}
  ContinueNext:=0
  If (obj=""){
    If (SubStr(value,0)!="]")
  ...
  Loop,Parse,value
  {
    If ((Quote=""""&&Default_Lib_Yaml_Quote(LF,A_LoopField,Quote,Bracket,Escape)) || (ContinueNext && !ContinueNext:=0))
      Continue
    If (Quote){
    ...
    If (obj=""){
      If !VQ
        If (Asc(LF)=91 && !Default_Lib_Yaml_Seq("","",LF))
          ||(Asc(LF)=123 && !Default_Lib_Yaml_Map("","",LF))
          Return 0
    } else {
      If (VQ || !Default_Lib_Yaml_SeqMap(cObj,"",LF))
        cObj.Insert(VQ?Default_Lib_Yaml_UniChar(LF):Trim(LF))
    }
    LF:="",VQ:=""
  }
  If (LF){
    If (obj=""){
      If !VQ
        If (Asc(LF)=91 && !Default_Lib_Yaml_Seq("","",LF))||(Asc(LF)=123 && !Default_Lib_Yaml_Map("","",LF))
          Return 0
    } else If (VQ || !Default_Lib_Yaml_SeqMap(cObj,"",LF))
      cObj.Insert(VQ?Default_Lib_Yaml_UniChar(LF):Trim(LF))
  }
  Return (obj=""?(Quote Bracket=""):1)
}
Default_Lib_Yaml_Map(obj,key,value,isVal=0){
  static base:={Dump:"Default_Lib_Yaml_Dump",Save:"Default_Lib_Yaml_Save",Add:"Default_Lib_Yaml_Add",Merge:"Default_Lib_Yaml_Merge",__Delete:"__Delete",_Insert:"_Insert",_Remove:"_Remove",_GetCapacity:"_GetCapacity",_SetCapacity:"_SetCapacity",_GetAddress:"_GetAddress",_MaxIndex:"_MaxIndex",_MinIndex:"_MinIndex",_NewEnum:"_NewEnum",_HasKey:"_HasKey",_Clone:"_Clone",Insert:"Insert",Remove:"Remove",GetCapacity:"GetCapacity",SetCapacity:"SetCapacity",GetAddress:"GetAddress",MaxIndex:"MaxIndex",MinIndex:"MinIndex",NewEnum:"NewEnum",HasKey:"HasKey",Clone:"Clone",base:{__Call:"Default_Lib_Yaml_Call"}}
  ContinueNext:=0
  If (obj=""){
    If (SubStr(value,0)!="}")
  ...
  Loop,Parse,value
  {
    ...
    If (obj=""){
      If VQ=
        If (Asc(v)=91 && !Default_Lib_Yaml_Seq("","",v))
          ||(Asc(v)=123 && !Default_Lib_Yaml_Map("","",v))
          Return 0
    } else {
      If (VQ || !Default_Lib_Yaml_SeqMap(cObj,k,v))
        cObj[KQ?Default_Lib_Yaml_UniChar(k):k]:=(VQ?Default_Lib_Yaml_UniChar(v):Trim(v))
    }
    k:="",v:="",VQ:="",KQ:=""
  }
  If (k){
    If (obj=""){
      ...
      LF:=Trim(LF)
      If (VQ)
        LF:=SubStr(LF,2,StrLen(LF)-2),cObj[k]:=Default_Lib_Yaml_UniChar(LF)
      else If (!Default_Lib_Yaml_SeqMap(cObj,k,LF))
        cObj[k]:=Trim(LF)
    }
  }
  Return (obj=""?(Quote Bracket=""):1)
}
Default_Lib_Yaml_Incomplete(value){
  return (Asc(Trim(value,"`n" A_Tab A_Space))=91 && !Default_Lib_Yaml_Seq("","",Trim(value,"`n" A_Tab A_Space)))
			|| (Asc(Trim(value,"`n" A_Tab A_Space))=123 && !Default_Lib_Yaml_Map("","",Trim(value,"`n" A_Tab A_Space)))
}
Default_Lib_Yaml_IsSeqMap(value){
	return (Asc(Trim(value,"`n" A_Tab A_Space))=91 && Default_Lib_Yaml_Seq("","",Trim(value,"`n" A_Tab A_Space)))
			|| (Asc(Trim(value,"`n" A_Tab A_Space))=123 && Default_Lib_Yaml_Map("","",Trim(value,"`n" A_Tab A_Space)))
}
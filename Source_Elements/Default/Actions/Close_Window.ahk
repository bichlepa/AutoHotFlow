...
AllElementClasses.push("Action_Close_Window")
...
Element_getParameters_Action_Close_Window()
{
	...
	parametersToEdit.push({id: "WinCloseMethod"})
...
Element_getParametrizationDetails_Action_Close_Window(Environment)
{
	...
	parametersToEdit.push({type: "button", goto: "Action_Close_Window_ButtonWindowAssistant", label: lang("Import window identification")})
	
	parametersToEdit.push({type: "Label", label: lang("Method")})
	parametersToEdit.push({type: "Radio", id: "WinCloseMethod", default: 1, choices: [lang("Close"), lang("Kill")]})
	return parametersToEdit
}
...
Element_CheckSettings_Action_Close_Window(Environment, ElementParameters)
{	
...
{
	WinCloseMethod:=ElementParameters.WinCloseMethod
	...
	if tempWinid
	{
		...
		else if (WinCloseMethod = 1)
		{
			WinKill,ahk_id %tempWinid%
		}
